...
#
# 나누는 이유는 실패 범위를 줄이는 것이다. 1단계 응답이 온전하지 않으면 그 판정을 믿을 수
# 없으므로 2단계를 부르지 않고 예산을 아낀다. 처음 나눈 근거는 한 엔진의 무료 등급 한도가
# 전 항목을 한 번에 받는지 몰라서였는데, 엔진이 그 뒤로 여러 번 바뀌어 그 근거는 남아 있지
# 않다. 실패 범위를 줄이는 쪽이 지금 근거다. run.py 의 CI_STAGES 에 이어지는 설명이 있다.
