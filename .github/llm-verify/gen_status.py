...
def main():
    ...
    # 모델 이름을 여기에 박지 않는다.
    #
    # 박아 두면 엔진을 갈 때마다 이 표가 조용히 낡는다. 실제로 gemini-2.5-flash 라고 적힌 채
    # 엔진이 두 번 바뀌었고, 그 사이 이 표는 게이트가 안 돈다고도 말하고 있었다.
    # 이 표의 일은 게이트가 도는지를 말하는 것이지 어느 모델인지를 말하는 것이 아니다.
    w(f"| G-PR | LLM, CI 자동 | 안 함 | "
      f"{'워크플로 있음' if have['G-PR 호출자 워크플로'][0] else '**워크플로 없음**'}. "
      f"엔진과 모델과 인증은 common 의 `llm-verify.yml` 이 정하고, "
      f"시크릿 등록 여부는 로컬에서 확인할 수 없다 |")
