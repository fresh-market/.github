...
| `stage1` | backend 항목 수. **기본으로 판정하는 범위다** |
| `stage2` | common 과 infra 항목 수. `--full` 일 때만 판정한다 |
| `changed` | 변경 파일 수 |
...
작업 중 반복 실행하는 도구인데 그러면 쓸 수가 없다. 그래서 판정 대상 코드가 있는 backend 부터 본다.

```bash
./verify.sh HEAD          # 1단계만. backend 항목
./verify.sh HEAD --full   # 2단계까지. common 과 infra 항목
```

`--full` 은 PR 을 올리기 전이나 인프라 문서를 고쳤을 때 쓴다.
CI 의 G-PR 도 1단계만 보므로, **common 과 infra 항목은 여기서 `--full` 로 보지 않으면 아무도 안 본다.**
