...
```json
{"skip": "false", "code_changed": "true", "rules": "on_no_match", "changed": "11"}
```
...
전부 보면 기준 문서를 열두 개 남짓 읽어야 해서 **판정 한 번이 10만 토큰대로 올라간다.**
...
`$INFRA/docs/system-design/` 은 **사람이 읽는 문서다.** 판정에 쓰지 않는다.

판정에 필요한 값은 guideline 이 이미 뽑아 두었다.
`code-guideline.md` 3장이 타임아웃 다섯 개를 표로 갖고 있고,
`qa-reliability-guideline.md` 2장이 구간별 연결과 읽기 타임아웃을 표로 갖고 있다.
그 문서들은 활성 항목의 `doc` 이라 이미 판정 입력에 들어간다.
그러니 system-design 을 다시 여는 것은 같은 값을 두 번 싣는 일이고,
거기 붙은 "왜 그 값인가" 라는 서술이 판정의 주의를 흩뜨린다.

`code-guideline.md` 가 스스로 그렇게 선을 그었다.

> 저쪽이 근거와 확정값을 담고, 이 문서는 그중 **PR 단위로 판정 가능한 것만 추출한다.**

각 장의 `출처:` 줄은 사람이 근거를 찾아가는 표시이지 판정할 때 따라가라는 뜻이 아니다.

**guideline 에 값이 없으면 그것은 판정할 수 없는 항목이다.**
system-design 을 뒤져 메우지 말고 `INSUFFICIENT_EVIDENCE` 로 두고 그 사실을 보고한다.
값을 판정에 쓰려면 guideline 으로 옮기는 것이 맞는 순서다.
...
**무엇이 위반인지만 적고 어떻게 고치라는 말은 쓰지 않는다.** 고칠 자리는 파일과 줄이
가리키고, 무엇이 기준인지는 항목이 가리키는 문서에 있다. 한 문장짜리 조치는 일반론에
머물러 그 둘 위에 얹는 값이 작다. G-PR 도 같은 제약을 받는다 (`run.py` 의 SYSTEM 규칙 10).
...
|---|---|---|
| 판정 주체 | `--agent` 로 준 CLI | CI 가 고정한 CLI 와 모델 |
