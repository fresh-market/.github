...
주제는 **대규모 트래픽 선착순 쿠폰 발급 시스템**이다. 재고 10,000장에 20,000명이 동시에 요청해도 **초과 발급 0건과 1인 최대 1매**를 지켜야 하고, 발급 이력 **300만 건 전체**를 대상으로 정합성을 검증하되 재실행하면 같은 결과가 나와야 한다.

**요구사항이 지연을 재지 않아서 팀은 요구 부하에서 발급 응답 p99 1초 이하라는 SLO 를 하나 더 걸었다.** 팀은 그 한 줄에서 안쪽 값들을 거꾸로 깎아 내려갔다.
