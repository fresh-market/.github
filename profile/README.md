...
**요구사항이 지연을 재지 않아서 팀은 요구 부하에서 발급 응답 p99 1초 이하라는 SLO 를 하나 더 걸었다.** 팀은 그 한 줄에서 안쪽 값들을 거꾸로 깎아 내려갔다. 아래 다섯이 선착순 발급에서 가장 품이 든 대목이다.
...
**차가운 JVM 은 요청을 처리하면서 동시에 자기를 컴파일한다.** 90초 회차 동안 JIT 컴파일 증가분이 차가움 49,676ms 대 데워짐 11,113ms 였다. 그 시간은 **어느 타임아웃도 덮지 않는 구간**이다.
